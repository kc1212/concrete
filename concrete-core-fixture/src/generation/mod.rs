...
//! fixtures.
//!
//! Generating the pre-execution context for the fixtures can be tricky. We have to generate
//! entities of the right type, which are also functionally compatible (encoded with the same secret
//! key for instance). The [`Maker`] type is the main entry point to context setup, which is a two
//! steps process:
//!
...
//!
//! Of course, we can also go in the inverse direction by _unsynthesizing_ entities into
