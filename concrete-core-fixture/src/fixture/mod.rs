...
//! + Generating random raw inputs (where _raw_ ~ unsigned integers derivatives)
...
//! trait do not fix the `Engine` and `Precision` type parameters, but rather restrict them to a
...
/// A trait for types implementing a fixture for a particular engine traits.
///
/// To understand how the differents pieces fit, see how the default methods `sample`, `test`,
...
pub trait Fixture<Precision: IntegerPrecision, Engine: AbstractEngine, RelatedEntities> {
    ...
    /// A type containing prototypical values (such as secret keys) passed around the engine
    /// execution.
    type Bypass;
    ...
    /// A method which generate a random raw input.
    fn generate_random_raw_inputs(parameters: &Self::Parameters) -> Self::RawInputs;

    /// A method which prepare the pre-execution context for the engine execution.
    ///
    /// The first returned value (the bypass), can be used to store prototypical secret keys used in
    /// the `process_context` method to decrypt values in the output context.
    fn prepare_context(
    ...
    /// A method which execute the engine in the pre-execution context, returning the post-execution
    /// context.
    fn execute_engine(
        parameters: &Self::Parameters,
        engine: &mut Engine,
    ...
    /// A method which processes the post-execution context, and return the raw outputs.
    fn process_context(
        parameters: &Self::Parameters,
        maker: &mut Maker,
        bypass: Self::Bypass,
    ...
    /// A method which generate a sample of engine execution, for a fixed set of raw inputs and a
