...
engine_error! {
    GgswCiphertextDiscardingEncryptionError for GgswCiphertextDiscardingEncryptionEngine @
...
impl<EngineError: std::error::Error> GgswCiphertextDiscardingEncryptionError<EngineError> {
...
/// A trait for engines encrypting (discarding) GGSW ciphertexts.
///
/// # Semantics
///
...
///
/// # Formal Definition
pub trait GgswCiphertextDiscardingEncryptionEngine<SecretKey, Plaintext, Ciphertext>:
    AbstractEngine
where
    SecretKey: GlweSecretKeyEntity,
    ...
    Ciphertext: GgswCiphertextEntity<KeyDistribution = SecretKey::KeyDistribution>,
{
    ...
    /// of [`GgswCiphertextDiscardingEncryptionError`]. For safety concerns _specific_ to an engine,
