...
engine_error! {
    GgswCiphertextTrivialEncryptionError for GgswCiphertextTrivialEncryptionEngine @
}

/// A trait for engines trivially encrypting GGSW ciphertext.
///
/// # Semantics
///
...
pub trait GgswCiphertextTrivialEncryptionEngine<Plaintext, Ciphertext>: AbstractEngine
where
    ...
    Ciphertext: GgswCiphertextEntity,
{
    ...
    fn trivially_encrypt_ggsw_ciphertext(
        ...
        input: &Plaintext,
    ) -> Result<Ciphertext, GgswCiphertextTrivialEncryptionError<Self::EngineError>>;

    /// Unsafely creates the trivial GGSW encryption of the plaintext vector.
    ///
    /// # Safety
    /// For the _general_ safety concerns regarding this operation, refer to the different variants
    /// of [`GgswCiphertextTrivialEncryptionError`]. For safety concerns _specific_ to an engine,
