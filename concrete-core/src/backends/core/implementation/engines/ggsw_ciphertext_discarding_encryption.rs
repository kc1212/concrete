...
use crate::specification::engines::{
    GgswCiphertextDiscardingEncryptionEngine, GgswCiphertextDiscardingEncryptionError,
};

/// # Description:
/// Implementation of [`GgswCiphertextDiscardingEncryptionEngine`] for [`CoreEngine`] that operates
/// on 32 bits integers.
impl GgswCiphertextDiscardingEncryptionEngine<GlweSecretKey32, Plaintext32, GgswCiphertext32>
    for CoreEngine
{
    /// # Example:
    /// ```
    ...
    /// let mut ciphertext =
    ///     engine.encrypt_ggsw_ciphertext(&key_1, &plaintext, noise, level, base_log)?;
    ...
    ///
    /// engine.discard_encrypt_ggsw_ciphertext(&key_2, &mut ciphertext, &plaintext, noise)?;
    /// #
    ...
    /// # }
    /// ```
    fn discard_encrypt_ggsw_ciphertext(
        &mut self,
        key: &GlweSecretKey32,
        output: &mut GgswCiphertext32,
        input: &Plaintext32,
        noise: Variance,
    ) -> Result<(), GgswCiphertextDiscardingEncryptionError<Self::EngineError>> {
        GgswCiphertextDiscardingEncryptionError::perform_generic_checks(key, output)?;
        unsafe { self.discard_encrypt_ggsw_ciphertext_unchecked(key, output, input, noise) };
        Ok(())
    }

    unsafe fn discard_encrypt_ggsw_ciphertext_unchecked(
        &mut self,
        key: &GlweSecretKey32,
        output: &mut GgswCiphertext32,
        input: &Plaintext32,
...
/// Implementation of [`GgswCiphertextDiscardingEncryptionEngine`] for [`CoreEngine`] that operates
/// on 64 bits integers.
impl GgswCiphertextDiscardingEncryptionEngine<GlweSecretKey64, Plaintext64, GgswCiphertext64>
    for CoreEngine
{
    /// # Example:
    /// ```
