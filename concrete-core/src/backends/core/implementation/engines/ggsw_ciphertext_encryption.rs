...
use crate::specification::engines::{
    GgswCiphertextEncryptionEngine, GgswCiphertextEncryptionError,
};
use crate::specification::entities::GlweSecretKeyEntity;

/// # Description:
/// Implementation of [`GgswCiphertextEncryptionEngine`] for [`CoreEngine`] that operates on 32 bits
/// integers.
impl GgswCiphertextEncryptionEngine<GlweSecretKey32, Plaintext32, GgswCiphertext32> for CoreEngine {
    /// # Example:
    /// ```
    /// use concrete_commons::dispersion::Variance;
    ...
    ) -> Result<GgswCiphertext32, GgswCiphertextEncryptionError<Self::EngineError>> {
        Ok(unsafe {
            self.encrypt_ggsw_ciphertext_unchecked(
                key,
                input,
                noise,
    ...
    unsafe fn encrypt_ggsw_ciphertext_unchecked(
        &mut self,
        key: &GlweSecretKey32,
        input: &Plaintext32,
        noise: Variance,
...
/// # Description:
/// Implementation of [`GgswCiphertextEncryptionEngine`] for [`CoreEngine`] that operates on 64 bits
/// integers.
impl GgswCiphertextEncryptionEngine<GlweSecretKey64, Plaintext64, GgswCiphertext64> for CoreEngine {
    /// # Example:
    /// ```
    /// use concrete_commons::dispersion::Variance;
    /// use concrete_commons::parameters::{
    ...
    fn encrypt_ggsw_ciphertext(
        ...
        decomposition_level_count: DecompositionLevelCount,
        decomposition_base_log: DecompositionBaseLog,
    ) -> Result<GgswCiphertext64, GgswCiphertextEncryptionError<Self::EngineError>> {
        Ok(unsafe {
            self.encrypt_ggsw_ciphertext_unchecked(
                key,
                input,
